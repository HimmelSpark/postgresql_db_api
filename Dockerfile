FROM ubuntu:16.04
MAINTAINER Adamyan Petros
RUN apt-get update -y
ENV PGVER 9.5
RUN apt-get install -y postgresql-$PGVER
USER postgres

RUN /etc/init.d/postgresql start &&        psql --command "CREATE USER petros WITH SUPERUSER PASSWORD '11111111';" && createdb -E UTF8 -T template0 -O petros forumdb && /etc/init.d/postgresql stop
...
CMD service postgresql start && java -Xms200M -Xmx200M -Xss256K -jar target/rha-0.0.1-SNAPSHOT.jar